...
p.readuntil("(first is highest aka more sensitive):\n")
level_map = {}
level_names = []
level_num = 40

for _ in range(40):
    level_names.append(p.recvline().decode().strip())

for name in level_names:
    level_map[name] = level_num
    level_num -= 1

p.readuntil("5 Categories:\n")
cat_map = []
for _ in range(5):
    cat_map.append(p.recvline().decode().strip())
